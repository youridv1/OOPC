...
void wall::draw(){
  for(int i = location.x; i <= location.x + size.x; i++)
  {
    ...
    {
      if(i == location.x || i == location.x + size.x || j == location.y || j == location.y + size.y || filled)
      {
        w.write( hwlib::xy(i, j) );
      }
    }
...
victim::victim( hwlib::window & w, const hwlib::xy & location, const hwlib::xy & size, hwlib::xy  bounce ):
  rectangle( w, location, size, bounce )
{}
...
void victim::draw(){
  for(int i = location.x; i <= location.x + size.x; i++)
  {
    for(int j = location.y; j <= location.y + size.y; j++)
    {
      if(i == location.x || i == location.x + size.x || j == location.y || j == location.y + size.y)
      {
        w.write( hwlib::xy(i, j) );
      }
    }
