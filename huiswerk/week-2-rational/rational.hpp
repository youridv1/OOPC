...
private:
  ...
  /// reduces rationals
  /// \details
  /// reduces a rational to its simplest form by determining the largest number by which the counter and denominator can be divided.
  /// Divides by the biggest common denominator.
   void reduce(){
      int a = counter;
      int b = denominator;
...
public:
   ...
   /// \details
   /// multiplies the counter and denominator of a rational by the counter and denominator of another rational.
   rational operator*=( const rational & rhs ){
      counter *= rhs.counter;
      denominator *= rhs.denominator;
      reduce();
